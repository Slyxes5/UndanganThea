/* =========================================================
   INVITATION APP — Best Version
   - Local /photo images
   - Gallery auto-build + autoplay + swipe + lightbox
   - Music, Countdown, Confetti (sleep when hidden)
   - Map by coordinates, Share, Add-to-Calendar
   - Guestbook (Apps Script) + anti-spam + fallback local
========================================================= */

/* ===== CONFIG ===== */
// Foto utama & galeri (pakai folder lokal /photo)
...
/* ===== Drive Fallback (not used for local but safe) ===== */
...
if (mainPhotoEl){
  if (PHOTO_URL) { mainPhotoEl.loading='lazy'; mainPhotoEl.decoding='async'; setImgSrcWithFallback(mainPhotoEl, PHOTO_URL); }
...
(function(){
  ...
  const ics=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Vathesya//Birthday//ID','BEGIN:VEVENT',
    ...
    'SUMMARY:Sweet Seventeen Vathesya','LOCATION:GMIM Marturia Roong, Tomohon','DESCRIPTION:See invite page for details.',
    'END:VEVENT','END:VCALENDAR'].join('\r\n');
  btn.href=URL.createObjectURL(new Blob([ics],{type:'text/calendar'}));
})();

/* ===== Reveal on scroll (Thank You) ===== */
...
(async function initGuestbook(){
  const bindSubmit=(sender)=>{ gbForm?.addEventListener('submit',async(e)=>{ e.preventDefault();
    ...
    const trap=document.getElementById('gbTrap'); if(trap?.value) return; // honeypot
